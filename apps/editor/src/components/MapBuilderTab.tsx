        ...
        <div className="controls-row">
          ...
          <div className="form-group">
            ...
            <select
              className="select"
              value={slot}
              onChange={(e) => setSlot(Number(e.target.value) as MapSlot)}
            >
              {([1, 2, 3, 4, 5] as MapSlot[]).map((s) => (
        ...
        <div className="controls-row" style={{ marginTop: 8 }}>
          <div className="form-group" style={{ flex: 1 }}>
            ...
            <select
              className="select"
              value={savedMapId}
              onChange={(e) => setSavedMapId(e.target.value)}
            >
              <option value="">-- select map --</option>
              {maps.map((m) => (
                <option key={m.id} value={m.id}>
                  {m.name} ({m.theme}, slot {m.slot})
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="palette-list">
            {paletteAssets.length === 0 && (
              <p style={{ color: 'var(--muted)', fontSize: 12 }}>
                No assets for this theme. Create some in the Assets tab.
              </p>
            )}
            {paletteAssets.map((a) => (
              <div
        ...
        <div>
          <div
            ...
            style={{
              width: CANVAS_W,
              height: CANVAS_H,
              cursor: paletteAsset ? 'crosshair' : 'default',
            }}
            ...
            {entities.map((ent, i) => {
              ...
              return (
                <div
                  ...
                  onClick={(e) => {
                    e.stopPropagation();
                    setSelectedEntityIdx(i === selectedEntityIdx ? null : i);
                  }}
                >
                  {a?.imageDataUrl ? (
                    <img
          ...
          <div className="controls-row" style={{ marginTop: 8 }}>
            ...
            <span style={{ color: 'var(--muted)', fontSize: 12, marginLeft: 'auto' }}>
              ...
              {paletteAsset ? ` • placing: ${paletteAsset.name}` : ' • click palette to select'}
            </span>
          </div>
        </div>
      </div>
